...
public final class RsaCodingUtil {
	...
	public static String encryptByPublicKey(String src, PublicKey publicKey) {
		...
		return RsaConst.byte2Hex(destBytes);
	}
	...
	public static String decryptByPrivateKey(String src, PrivateKey privateKey) {
		...
		try {
			byte[] destBytes = rsaByPrivateKey(RsaConst.hex2Bytes(src), privateKey, Cipher.DECRYPT_MODE);
			if (destBytes == null) {
				return null;
			}
...
//			//log.error("解密内容异常", e);
	...
	public static String decryptByPublicKey(String src, PublicKey publicKey) {
		...
		try {
			byte[] destBytes = rsaByPublicKey(RsaConst.hex2Bytes(src), publicKey, Cipher.DECRYPT_MODE);
			if (destBytes == null) {
				return null;
			}
			return new String(destBytes, RsaConst.ENCODE);
	 ...
	 /** 
	    ...
	    */  
	    private static void generateKeyPair() throws Exception {  
	        ...
	        System.out.println("ras私钥:" + privateKeyBase64);
	    }  

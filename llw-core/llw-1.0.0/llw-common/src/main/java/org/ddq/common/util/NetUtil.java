...
public class NetUtil {
	...
	private static InetAddress getLocalAddress0() {
		...
		try {
			...
			LogerUtil.warn("Failed to retriving ip address, " + e.getMessage(), e);
		}
		try {
			...
			LogerUtil.log("Failed to retriving ip address, " + e.getMessage(), e);
		}
		LogerUtil.error(NetUtil.class,"Could not get local host ip address, will use 127.0.0.1 instead.");
		return localAddress;
	}

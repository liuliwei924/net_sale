...
@Lazy
@Service
public class RuleConditionService extends BaseService {
	static final String NAMESPACE = "RULECONDITION";
	...
	public AppResult delete(AppParam context) {
		...
		if (!StringUtils.isEmpty(ids)) {
			for (String id : ids.split(",")) {
				...
				param.addAttr("conditionId", id);
				param.setDataBase(DBConst.Key_sys_DB);
				result = super.delete(param, NAMESPACE);
			}
		} else if (context.getAttr("conditionId") != null) {

...
@Lazy
@Service
public class JobProcessService extends BaseService {
	private static final String NAMESPACE = "JOBPROCESS";
	...
	public AppResult insert(AppParam context) {
		...
		result.putAttr("processId", context.getAttr("processId"));
		return result;
	}
	...
	public AppResult delete(AppParam context) {
		...
		if (!StringUtils.isEmpty(ids)) {
			for (String id : ids.split(",")) {
				...
				param.addAttr("processId", id);
				
				result = super.delete(param, NAMESPACE);
			}
		} else if (!StringUtils.isEmpty(context.getAttr("processId"))) {

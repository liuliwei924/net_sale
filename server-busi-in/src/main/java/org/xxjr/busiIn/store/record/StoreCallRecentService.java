...
@Lazy
@Service
public class StoreCallRecentService extends BaseService {
	private static final String NAMESPACE = "STORECALLRECENT";
	...
	public AppResult saveOrUpdate(AppParam params) {
		...
		result.putAttr(DuoduoConstant.TOTAL_SIZE, DuoduoConstant.DAO_Update_SIZE);
		if((Integer)result.getAttr(DuoduoConstant.DAO_Update_SIZE) == 0){
			result = this.insert(params);
			result.putAttr(DuoduoConstant.TOTAL_SIZE, DuoduoConstant.DAO_Insert_SIZE);
		}

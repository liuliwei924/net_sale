...
@Lazy
@Service
public class StorePauseAllotService extends BaseService {
	private static final String NAMESPACE = "STOREPAUSEALLOT";
	 ...
	 * @return
	 */
	public AppResult storePauseAllotCount(AppParam params) {
		String tableName = StoreOptUtil.getTableName(null);
		params.addAttr("tableName", tableName);
		//暂停分单日期
		String pauseDate = StringUtil.getString(params.getAttr("pauseDate"));
		//暂停分单年月
		String pauseMonth = DateUtil.toStringByParttern(DateUtil.toDateByString(
				pauseDate, DateUtil.DATE_PATTERN_YYYY_MM_DD), DateUtil.DATE_PATTERN_YYYYMM);
		String currentMonth = DateUtil.toStringByParttern(new Date(), DateUtil.DATE_PATTERN_YYYYMM);
		int compareNum = currentMonth.compareTo(pauseMonth);
		if(compareNum > 0){
			String tableName1 = StoreOptUtil.getTableName(pauseDate);
			params.addAttr("tableName1", tableName1);
		}
		return super.query(params, NAMESPACE,"storePauseAllotCount");
	}
